...
New \texttt{Kokkos\_ENABLE\_DEPRECATED\_CODE\_5} (\texttt{= ON} by default)
  \begin{itemize}
...
\begin{frame}[fragile]{Removed options and Version bumps}
\textbf{Makefiles}
\begin{itemize}
  \item Deprecated in 4.6, dropped in 5.0. %#8374
  \item Removes the burden of maintaining two build systems
\end{itemize}
\textbf{Kepler architecture (Nvidia GPU)}
\begin{itemize}
...
\begin{itemize}
  \item v1.8.3 is the new minimum. %#8579
\end{itemize}
\end{frame}
...
\end{itemize}
\textbf{Parallel dispatch: \texttt{parallel\_for/scan/reduce}}
\begin{itemize}
  \item Dispatch only within the Kokkos execution environment.
  \item Abort if used before \texttt{initialize()} or after \texttt{finalize()}. %#7675
\end{itemize}
\textbf{Execution space chosen by default in \texttt{Kokkos::Graph::submit()}}
\begin{itemize}
  \item Used to be the execution space specified during \texttt{Graph} construction.
  \item Now, defaults to \texttt{Kokkos::DefaultExecutionSpace}. %#8365
  \item Matches semantics of other parallel constructs in Kokkos.
\end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Breaking changes}
\textbf{Default SIMD vector width}
\begin{itemize}
...
\end{itemize}
\textbf{Memory trait alias \texttt{Kokkos::MemoryRandomAccess}}
\begin{itemize}
