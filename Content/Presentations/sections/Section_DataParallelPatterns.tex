...
\begin{frame}[fragile]{Exercise \#1: From a serial loop to a Kokkos lambda}

  \begin{columns}[T,onlytextwidth]
    \column{0.47\textwidth}
    \textbf{Serial outer loop}
    \begin{tiny}
...
Kokkos::parallel_reduce(N,
  ...
  \begin{block}{Mapping the loop to a lambda}
    \begin{small}
    \begin{itemize}
      \item \texttt{N} defines the iteration range; \texttt{j} is the lambda's work index.
      \item \texttt{KOKKOS\_LAMBDA} captures \texttt{A}, \texttt{x}, and \texttt{y} by value.
      \item Kokkos combines each private \texttt{update} into the final \texttt{result}.
    \end{itemize}
    \end{small}
  \end{block}
