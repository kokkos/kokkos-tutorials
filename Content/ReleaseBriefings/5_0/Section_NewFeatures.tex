...
\begin{itemize}
\item Had to disable in Trilinos (and thus Spack) anyway due to needed Trilinos refactor
\begin{itemize}
  \item They used lots of \texttt{Impl} namespace features from Kokkos
\end{itemize}
\item Got reports in 4.7.0 of performance regressions
\begin{itemize}
  \item Addressed with workaround for implicit integer conversion when indexing into \texttt{View}
  \item Where is a bit more pain to come to clean up 32bit vs 64bit indexing
\end{itemize}
\item More issues when hoped for with older compilers
\begin{itemize}
  \item Most are not allowed anymore due to C++20 requirements
  \item We are getting initial reports about issues with GCC 10.4 as well as some older CUDA (before 12.6) in corner cases
\end{itemize}
\end{itemize}
