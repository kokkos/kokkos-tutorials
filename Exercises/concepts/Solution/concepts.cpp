...
template<Kokkos::ExecutionSpace Exec, AnyView Vector>
requires(Vector::rank() == 1)
void init_view(Exec exec, Vector x, typename Vector::element_type val) {
  Kokkos::parallel_for("init_vector", Kokkos::RangePolicy(exec, 0, x.extent_int(0)),
    KOKKOS_LAMBDA(int i) { x(i) = val; });
}

template<Kokkos::ExecutionSpace Exec, AnyView Matrix>
requires(Matrix::rank() == 2)
void init_view(Exec exec, Matrix A, typename Matrix::element_type val) {
...
template<class V>
concept AnyVector = Kokkos::is_view_v<V> &&
                    std::is_arithmetic_v<typename V::element_type> &&
                    V::rank() == 1;

template<class V>
concept AnyMatrix = Kokkos::is_view_v<V> &&
                    std::is_arithmetic_v<typename V::element_type> &&
                    V::rank() == 2;

template<Kokkos::ExecutionSpace Exec, AnyMatrix Matrix, AnyVector VectorX, AnyVector VectorY>
double run_code(Exec exec, Matrix A, VectorX x, VectorY y) {  
  ...
  static_assert(
    Matrix::static_extent(0) == Kokkos::dynamic_extent ||
    VectorY::static_extent(0) == Kokkos::dynamic_extent ||
    Matrix::static_extent(0) == VectorY::static_extent(0));
  
  static_assert(
    Matrix::static_extent(1) == Kokkos::dynamic_extent ||
    VectorX::static_extent(0) == Kokkos::dynamic_extent ||
    Matrix::static_extent(1) == VectorX::static_extent(0));
