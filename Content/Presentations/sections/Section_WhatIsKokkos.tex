...
\begin{frame}[fragile]{The HPC Hardware Landscape}
  ...
  \begin{scriptsize}
  \emph{(a)} Initially not working. Now more robust for Fortran than C\texttt{++}, but getting better.
...
\begin{frame}[fragile]{What is Kokkos?}
	\begin{itemize}
		\item A C\texttt{++} Programming Model for Performance Portability
			\begin{itemize}
				\item Implemented as a template library on top CUDA, HIP, OpenMP, ...
				\item Aims to be descriptive not prescriptive
				\item Aligns with developments in the C\texttt{++} standard
			\end{itemize}
		...
		\item It is an Open Source project with a growing community
			\begin{itemize}
				...
				\item Thousands of users at hundreds of international institutions, \emph{e.g.}\ academia and governmental funded organizations
			\end{itemize}
	\end{itemize}
\end{frame}

\begin{frame}[fragile]{Emerging economics in the world of AI}

We don't know the long-term costs
of maintaining AI-generated code in scientific HPC applications
\begin{itemize}
\item \textbf{Hypothesis 1:}
  Kokkos AI-generated code is
  \textbf{likely cheaper} because the Kokkos model is
  more compact,
  high level,
  and a single Kokkos code
  supports multiple backends (CUDA, ROCm, SYCL, CPU, etc.)
  that \textbf{translates to fewer LOC and fewer tokens}
\item \textbf{Hypothesis 2:} Kokkos AI-generated code is
  \textbf{likely more constrained and therefore more performant}
  than equivalent backend AI-generated code
\end{itemize}

\pause

\begin{block}{More study needed}
  We are not aware of studies and this is conjecture,
  nevertheless for the above reasons
  we expect Kokkos to be useful in an AI world.
\end{block}

\end{frame}

\begin{frame}{High Performance Software Foundation (HPSF)}
\vspace{0.125cm}
\begin{columns}[T,onlytextwidth]
    % Left column
    \begin{column}{0.48\textwidth}
        \centering
        \includegraphics[width=0.5\linewidth]{../modularized/figures/linux-foundation}
     \end{column}
\hfill
    % Right column
    \begin{column}{0.48\textwidth}
        \centering
        \includegraphics[width=0.5\linewidth]{../modularized/figures/hpsf-logo}
      \end{column}
\end{columns}
\vspace{0.5cm}
\begin{columns}[T,onlytextwidth]
    ...
    % Left column
    \begin{column}{0.48\textwidth}
    HPSF: The home for Open Source projects in the HPC space
    \begin{itemize}
    \item Neutral Governance for projects
    \item Professional Foundation support from LF
	 \item Tools for sustaining communities
     \item An engineering community
    \end{itemize}
...
\hfill
    ...
    \begin{column}{0.48\textwidth}
        \centering
        \includegraphics[scale=0.09375]{../modularized/figures/hpsf-members}
      \end{column}
\end{columns}
\begin{center}
\tiny HPSF sponsors by membership tier as of September 2026
\end{center}
\end{frame}
...
\hfill
    ...
    \begin{column}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{../modularized/figures/kokkos-contributors}
      \end{column}
\end{columns}

\end{frame}

\begin{frame}{Kokkos Community Evolution}
User growth outside US has accelerated in particular in France
\begin{itemize}
\item Hard to tell but seems to be still exponential (doubling in 2 ¼ years)??
\end{itemize}
Significant diversifying of user base over the last few years
\begin{itemize}
\item DOE is not a clearly dominating factor in user base anymore
\item Growing impact with industry – in particular in the US
\end{itemize}

\includegraphics[width=\linewidth]{../modularized/figures/kokkos-slack-members}
\end{frame}
...
\begin{frame}[fragile]{Kokkos and the C\texttt{++} Standard}
  \textbf{Kokkos helps improve ISO C\texttt{++}}
...
\end{center}
	...
	\vspace{-25pt}
\textit{Ten current or former Kokkos team members are members of the ISO C\texttt{++} standard committee.}
\end{frame}

\iffull
\begin{frame}[fragile]{C\texttt{++}20 std::atomic\_ref}
	\textbf{C\texttt{++}11 std::atomic insufficient for HPC}
	\begin{itemize}
	...
	\textbf{C\texttt{++}20 std::atomic\_ref adds atomic capabilites as in Kokkos}
	\begin{itemize}
...
\iffull
\begin{frame}[fragile]{C\texttt{++}23 std::mdspan}
   \textbf{C\texttt{++} does not provide multi dimensional arrays}
	\begin{itemize}
           \item Every scientific programming language has them: Fortran, Matlab, Python, ... 
	\end{itemize}

	\textbf{C\texttt{++}23 std::mdspan adds Kokkos::View like arrays}
	\begin{itemize}
...
%    \begin{itemize}
%       \item 41 list C\texttt{++} as critical
