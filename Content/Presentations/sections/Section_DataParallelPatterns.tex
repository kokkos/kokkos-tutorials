...
\begin{frame}[fragile]{Using Kokkos for data parallel patterns (2)}
  ...
  \hspace{10pt} As \textbf{functors} or \textit{function objects}, a common pattern in C\texttt{++}.
...
\begin{frame}[fragile]{Using Kokkos for data parallel patterns (7)}
  ...
  Functors are tedious $\Rightarrow$ \ul{\textbf{C\texttt{++}11 Lambdas} are concise}
...
@gray// atomForces already exists
// data already exists
...
@gray);@gray
  ...
  (Will review C\texttt{++} Lambdas on the next two slides)
  ...
  \begin{alertblock}{Warning: Lambda capture and C\texttt{++} containers}
...
\begin{frame}[fragile]{Quick review of C\texttt{++} Lambda syntax (1)}
...
\begin{code}[keywords={}, frame=single, numbers=left, firstnumber=3, stepnumber=4]
    // explicit, but gives an error!  why?
    [@pattern&atomForces@pattern] (const int64_t atomIndex) {
\end{code}

\pause

\begin{code}[keywords={}, frame=single, numbers=left, firstnumber=3, stepnumber=4]
    // explicit, now captures all needed variables.
    [@pattern&atomForces, &data@pattern] (const int64_t atomIndex) {
\end{code}
...
\begin{frame}[fragile]{Quick review of C\texttt{++} Lambda syntax (2)}
  \url{https://cppreference.com/cpp/language/lambda}
...
\begin{code}[keywords={}, frame=single, numbers=left, firstnumber=4]
    [@patternatomForces, data@pattern] (const int64_t atomIndex) { // explicit
\end{code}

\begin{block}{Important concept}
  Kokkos uses capture by value (that makes a copy)
    to avoid dangling or inaccessible references when dispatching parallel operations.
\end{block}

In the next exercise, we see \textbf{these copies are immutable}.
...
\begin{frame}[fragile]{Amdahl's Law (2)}
  ...
  \begin{center}
    % Reproduction sources and instructions:
    % ../benchmarks/ScalarIntegration/README.md
