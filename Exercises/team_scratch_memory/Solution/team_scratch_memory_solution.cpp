...
int main( int argc, char* argv[] )
{
  ...
  using ScratchViewType = Kokkos::View<double*,
  ...
  int scratch_size = ScratchViewType::shmem_size( N );
  ...
  for ( int repeat = 0; repeat < nrepeat; repeat++ ) {
    ...
    Kokkos::parallel_reduce( "yAx", team_policy( E, Kokkos::AUTO, 32 ).set_scratch_size( 0, Kokkos::PerTeam( scratch_size ) ),
      ...
      ScratchViewType s_x( teamMember.team_scratch( 0 ), N );

      if ( teamMember.team_rank() == 0 ) {
        Kokkos::parallel_for( Kokkos::ThreadVectorRange( teamMember, N ), [&] ( const int i ) {
          s_x( i ) = x( e, i );
        });
      }
      ...
      Kokkos::parallel_reduce( Kokkos::TeamThreadRange( teamMember, M ), [&] ( const int j, double &innerUpdateM ) {
        double tempN = 0;

        Kokkos::parallel_reduce( Kokkos::ThreadVectorRange( teamMember, N ), [&] ( const int i, double &innerUpdateN ) {
          innerUpdateN += A( e, j, i ) * s_x( i );
        }, tempN );

        innerUpdateM += y( e, j ) * tempN;
      }, tempM );
