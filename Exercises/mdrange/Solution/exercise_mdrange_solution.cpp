...
int main( int argc, char* argv[] )
{
  ...
  Kokkos::parallel_for( "init_y", range_policy(0,M), KOKKOS_LAMBDA ( const int i ) {
      y(i) = 1;
    }
  );

  // Initialize x vector on device
  Kokkos::parallel_for( "init_x", range_policy(0,N), KOKKOS_LAMBDA ( const int i ) {
      x(i) = 1;
    }
  );
  ...
  Kokkos::parallel_for( "init_A", mdrange_policy({0,0}, {M,N}), KOKKOS_LAMBDA ( const int j , const int i ) {
      A(j,i) = 1;
    }
  );

  // Wait for initalization calls to finish
  Kokkos::fence();
  ...
  for ( int repeat = 0; repeat < nrepeat; repeat++ ) {
    ...
    Kokkos::parallel_reduce( M, KOKKOS_LAMBDA ( int j, double &update ) {
      double temp2 = 0;
